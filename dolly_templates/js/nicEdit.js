...
var nicEditorInstance = bkClass.extend({
  ...
  construct: function(e, options, nicEditor) {
    ...
    if (isTextarea || this.options.hasPanel) {
      var ie7s = (bkLib.isMSIE && !((typeof e.ownerDocument.body.style.maxHeight != "undefined") && e.ownerDocument.compatMode == "CSS1Compat"));
      var s = {
        width: newX + 'px',
      ...
      this.editorContain = new bkElement('DIV', e.ownerDocument).setStyle(s).appendBefore(e);
      var editorElm = new bkElement('DIV', e.ownerDocument).setStyle({
        width: (newX - 8) + 'px',
        margin: '4px',
        minHeight: newY + 'px'
  ...
  init: function() {
    ...
    this.instanceDoc = this.e.ownerDocument.defaultView;
  ...
  getSel: function() {
	var doc = this.e.ownerDocument, win = doc.defaultView;
  ...
  getRng: function() {
    ...
    return (s.rangeCount > 0) ? s.getRangeAt(0) :
      s.createRange && s.createRange() || this.e.ownerDocument.createRange();
  },

  selRng: function(rng, s) {
    if (this.e.ownerDocument.defaultView.getSelection) {
      s.removeAllRanges();
      s.addRange(rng);
    } else {
      rng.select();
  ...
  nicCommand: function(cmd, args) {
    this.e.ownerDocument.execCommand(cmd, false, args);
  }
});
...
var nicEditorPanel = bkClass.extend({
  construct: function(e, options, nicEditor) {
    ...
    this.panelContain = new bkElement('DIV', e.ownerDocument).setStyle({
      overflow: 'hidden',
      width: '100%',
      border: '1px solid #fff',
      backgroundColor: '#fff'
    }).addClass('panelContain');
    this.panelElm = new bkElement('DIV', e.ownerDocument).setStyle({
      margin: '2px',
      marginTop: '0px',
      zoom: 1,
      overflow: 'hidden'
...
var nicEditorPane = bkClass.extend({
  construct: function(elm, nicEditor, options, openButton) {
    ...
    this.contain.noSelect().appendTo(elm.ownerDocument.body);
